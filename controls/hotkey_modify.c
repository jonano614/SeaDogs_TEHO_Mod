	...
	if(bOk)
	{
		if (sHKB == "HK_Musket")
		{
			...
			if(sGun != "" && HasSubStr(aGun.id, "mushket") && IsCharacterPerkOn(pchar,"Gunman") && CanEquipMushketOnLocation(PChar.Location))
			{
				if(!CheckAttribute(PChar, "IsMushketer")) 
				{
					...
					log_info("Выбрано огнестрельное оружие: "+GetConvertStr(aGun.name, "ItemsDescribe.txt"));
					PlaySound("PEOPLE\clothes1.wav");
				}
				else
				{
					Log_Info("Мушкет уже экипирован");
			...
			{
				// предупреждалки
				if(!IsCharacterPerkOn(pchar,"Gunman")) Log_Info("Для использования мушкета откройте личную способность");
				if(!CanEquipMushketOnLocation(PChar.Location)) Log_Info("Нельзя использовать мушкет"); // сейчас по факту это проверка модели героя
				if(pchar.model == "protocusto") Log_Info("Снимите водолазный скафандр");
				if(!HasSubStr(aGun.id, "mushket")) Log_Info("Мушкет отсутствует в инвентаре");
				PlaySound("interface\knock.wav");
			}
		}
		else
		{
			...
			if(sBlade != "")
			{
				...
				log_info("Выбрано холодное оружие: "+GetConvertStr(BladeInfo, "ItemsDescribe.txt"));
			}
			else
			{
				Log_Info("Клинок данного типа отсутствует в инвентаре");
				PlaySound("interface\knock.wav");
			}
		}
	}		
...
void HKB_Button(string sHKB) // боеприпас
{
	...
	log_info("Выбран боеприпас - "+GetConvertStr("itmname_"+BulletType, "ItemsDescribe.txt")+"");
	...
	if(GunBulletType != BulletType) log_info("Внимание!!! Боеприпас несовместим с Вашим текущим оружием");
	if(GunPowder != ""  && iGunPowder < 1) {log_info("Внимание!!! В инвентаре отсутствует порох"); PlaySound("interface\notebook.wav");}
}
