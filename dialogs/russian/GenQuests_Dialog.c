	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Convict_12_2_3":
			...
			else
			{
				...
				GenerateMerchant();
				makeref(MerPrm, MerchantParam);
				dialog.text = "Вот сразу видно щедрого человека, который не боится расставаться с деньгами, потому что знает, как их нужно добывать!\nПрошлой ночью мы видели, как контрабандисты грузили в лодки груз, очень похоже, что это - " + MerPrm.QuestGoodsIdx + ". Было темно, но на борту одной из лодок удалось прочесть название судна - '" + MerPrm.ShipName + "'... Корабль идет под флагом " + MerPrm.nation + ". Думается, это может вам пригодиться, хе-хе.";
				link.l1 = "Н-да? Может и пригодится... Ну, прощайте, следопыты.";
				link.l1.go = "exit";
				pchar.GenQuest.Convict = "close";
				AddQuestRecord("Convict", "7");
...
//				link.l1.go = "ChurchGenQuest1_DialogShip_5_2_1"; // Сваливаем, поверили ему
		...
		case "JusticeOnSale_3":
			dialog.text = "Наш капитан " + PChar.GenQuest.JusticeOnSale.SmugglerName + " попался патрулю из города " + XI_ConvertString("Colony" + PChar.GenQuest.JusticeOnSale.CityId + "Gen") + ", его бросили за решетку, но улик на него, чтобы к вдове тощей приставить, у них нет. Вытащить его самим нам не по силам\n" +
				"А вы человек уважаемый, вас в городе знают. Может, вы сможете договориться, внести залог, или просто выкупить его? Поверьте, мы сразу же уйдем, и никогда больше не вернемся на этот остров – ваша репутация ничуть не пострадает!"; // belamour gen
