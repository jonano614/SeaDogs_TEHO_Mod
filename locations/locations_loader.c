...
bool LoadLocation(ref loc)
{
	...
	//Set lighting path
	if(!CheckAttribute(&InterfaceStates, "DYNAMICLIGHTS") || !sti(InterfaceStates.DYNAMICLIGHTS))
	{
...
bool LocLoadModel(aref loc, string sat, string addition)
{
	...
	string tech = "";
	int level = 10;
	bool dynamicLightsOn = (CheckAttribute(&InterfaceStates, "DYNAMICLIGHTS")) && (sti(InterfaceStates.DYNAMICLIGHTS)); //  belamour динамический свет // mitrokosta фикс при отсутствии галки
	attr = sat + ".tech";
